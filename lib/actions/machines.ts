...
export async function submitMachineForm(
  ...
  try {
    ...
    const inspectionRecord: Partial<MachineInspectionRecord> = {
      ...
      locationTimestamp: formData.locationTimestamp
        ? admin.firestore.Timestamp.fromDate(
            new Date(formData.locationTimestamp)
          )
        : null,
    ...
    const docRef = await firestore.collection("machinetr").add(cleanedRecord);
...
export async function updateMachineInspectionRecord(
  ...
  try {
    // Debug logging
    console.log("updateMachineInspectionRecord called with:", {
      docId,
      updateData,
    });
      ...
      if (value !== null && value !== undefined) {
        ...
        if (key.endsWith("F") || key.endsWith("P") || key === "images") {
          if (
            ...
            console.warn(
              `Field ${key} should be array or string, got:`,
              typeof value,
              value
            );
          }
  ...
  try {
    ...
    if (allImageUrls.length > 0) {
      try {
        ...
        console.log(
          `Attempted to delete ${allImageUrls.length} total images from Storage (including question-specific images)`
        );
      } catch (storageError) {
...
export async function getMachinesWithInspectionsAction(
  ...
  success: boolean;
  machines?: MachineWithInspection[];
  error?: string;
}> {
  try {
    ...
    if (!machineSnapshot.empty) {
      console.log(
        "Sample machine data:",
        machineSnapshot.docs.slice(0, 2).map((doc) => ({
          id: doc.data().id,
          site: doc.data().site,
          type: doc.data().type,
          bu: doc.data().bu,
    ...
    if (machineSnapshot.empty) {
      ...
      console.log(
        `Machines with bu=${bu}, type=${type.toLowerCase()} (any site):`,
        debugSnapshot1.size
      );
      ...
      console.log(
        `Machines with bu=${bu}, site=${site} (any type):`,
        debugSnapshot2.size
      );

      if (!debugSnapshot2.empty) {
        const siteTypes = debugSnapshot2.docs.map((doc) => doc.data().type);
        console.log(`Available types for bu=${bu}, site=${site}:`, [
          ...new Set(siteTypes),
        ]);
      }
    ...
    inspectionSnapshot.docs.forEach((doc) => {
      ...
      const correspondingMachine = machineSnapshot.docs.find(
        (mDoc) => mDoc.data().id === record.id && mDoc.data().site === site
      );
      ...
      if (correspondingMachine) {
        ...
        if (!existingRecord) {
          ...
          const existingTime = existingRecord.timestamp
            ? new Date(existingRecord.timestamp).getTime()
            : 0;
    ...
    const machinesWithInspections: MachineWithInspection[] =
      machineSnapshot.docs.map((doc) => {
        ...
        if (inspection?.timestamp) {
          const date = inspection.timestamp.toDate
            ? inspection.timestamp.toDate()
            : new Date(inspection.timestamp);
    ...
    const serializedMachines = machinesWithInspections.map((machine) =>
      serializeFirestoreData(machine)
    );

    console.log(
      "Returning machines with inspections:",
      serializedMachines.length
    );
...
export async function getRecentInspectionsAction(
  ...
  try {
    ...
    const serializedMachines = recentMachines.map((machine) =>
      serializeFirestoreData(machine)
    );
...
export async function getMachinesAction(
  ...
  try {
    ...
    if (machineSnapshot.empty) {
      ...
      console.log(
        "Debug check - machines with same bu/site (any type):",
        debugSnapshot.size
      );
...
export async function getMachineByIdAction(
  ...
  try {
    ...
    const machineQuery = firestore
      .collection("machine")
      .where("bu", "==", bu)
      .where("type", "==", normalizeTypeForDisplay(type))
      .where("id", "==", decodedId);
...
export async function getMachineInspectionRecordsAction(
  ...
  try {
    ...
    const records: MachineInspectionRecord[] = inspectionSnapshot.docs.map(
      (doc) => {
        const recordData = doc.data();
