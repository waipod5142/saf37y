    ...
    if (visitorExists && visitorData) {
      ...
      if (
        ...
        if (existingTx.isCheckOut) {
          // Check visit duration before allowing checkout
          const checkInTime = existingTx.transactionData.checkInTimestamp?.toDate();
          const now = new Date();
          const durationSeconds = (now.getTime() - checkInTime.getTime()) / 1000;

          if (durationSeconds < 60) {
            const remainingSeconds = Math.ceil(60 - durationSeconds);
            toast.error(t.checkoutTooFastError(remainingSeconds));
