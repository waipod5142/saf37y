...
export default function MachineDetailClient({
  ...
  const [showDefectResponseModal, setShowDefectResponseModal] = useState<
    string | null
  >(null);
  const [isSubmittingResponse, setIsSubmittingResponse] = useState(false);
  const [responder, setResponder] = useState("");
  const [defectAnswers, setDefectAnswers] = useState<{
    [questionName: string]: string;
  }>({});
  const [defectFixImages, setDefectFixImages] = useState<{
  ...
  useEffect(() => {
    if (showDefectResponseModal) {
      const currentRecord = records.find(
        (r) => (r.docId || r.id) === showDefectResponseModal
      );
      if (currentRecord) {
        ...
        Object.keys(currentRecord).forEach((key) => {
          if (key.endsWith("A")) {
            ...
            const questionName = key.slice(0, -1);
            images[questionName] = currentRecord[key].map(
  ...
  const handleSubmitDefectResponse = async (
    ...
    try {
      ...
      const result = await updateMachineInspectionRecord(
        record.docId,
        updateData
      );
  ...
  const getQuestionGroups = (record: MachineInspectionRecord) => {
    ...
    Object.keys(record).forEach((key) => {
      if (key.endsWith("R")) {
        ...
        const questionName = key.slice(0, -1);
        if (!groups[questionName]) groups[questionName] = { status: null };
        groups[questionName].images = Array.isArray(record[key])
          ? record[key]
          : [record[key]];
      } else if (
        ![
  ...
  const getInspectionResult = (record: MachineInspectionRecord) => {
    ...
    const failedItems = resultKeys.filter(
      (key) => record[key] === "fail" || record[key] === false
    );
        ...
        <CardContent>
                ...
                {Object.keys(record)
                  .filter(
                    (key) =>
                      ...
                      key !== "locationAccuracy" &&
                      !key.endsWith("R") &&
                      !key.endsWith("P") // Exclude remark and image fields
                  )
                  .map((key, idx) => {
                    const value = record[key];
                    const isPassed =
                      value === "pass" || value === true || value === "Passed";
                    const isFailed =
                      value === "fail" || value === false || value === "Failed";
          ...
          {(() => {
            ...
            const photoFields = Object.keys(record)
              .filter(
                (key) =>
                  allowedPhotoFields.includes(key) &&
                  record[key] &&
                    ...
                    return (
                      <div key={field} className="bg-gray-50 p-3 rounded-lg">
                        <h5 className="text-xs font-medium text-gray-600 mb-2 capitalize">
                          {photoType} View ({photos.length} photo
                          {photos.length > 1 ? "s" : ""})
                        </h5>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setShowDefectResponseModal(record.docId || record.id)
                  }
              ...
              <div className="space-y-4">
                {Object.entries(getQuestionGroups(record))
                  .filter(
                    ([questionName, data]) =>
                      data.status === "fail" || data.status === false
                  )
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-medium text-sm text-red-800">
                          {questionMapping[questionName] ||
                            questionName.replace(/([A-Z])/g, " $1").trim()}
                        </span>
                      </div>
                      ...
                      {(record[questionName + "A"] ||
                        ...
                        <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                          ...
                          {record[questionName + "F"] &&
                            Array.isArray(record[questionName + "F"]) &&
                            record[questionName + "F"].length > 0 && (
                              <div>
                                <span className="text-xs font-medium text-green-700 block mb-2">
                                  Fix Evidence (
                                  {record[questionName + "F"].length}):
                                </span>
                                <div className="flex gap-2 flex-wrap">
    ...
    return (
      <div className="mb-6">
        <p className="text-sm text-red-500 mt-2">
          No inspection records found for this machine.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="mb-6">
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-semibold">
            Machine Inspection Records ({showAllRecords ? records.length : 1} of{" "}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full mx-4">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete this inspection record? This
      ...
      {showDefectResponseModal && (
        <Dialog
          open={!!showDefectResponseModal}
          onOpenChange={() => setShowDefectResponseModal(null)}
        >
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            {(() => {
              ...
              const failedQuestions = Object.entries(
                getQuestionGroups(currentRecord)
              ).filter(
                ([questionName, data]) =>
                  data.status === "fail" || data.status === false
              );
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <p className="text-xs text-blue-600 mt-1">
                      This person will be recorded as the defect responder for
                      all fixes in this record.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <h4 className="font-semibold text-gray-700">
                      Fix Details for {failedQuestions.length} Failed Item
                      {failedQuestions.length > 1 ? "s" : ""}
                    </h4>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            Upload photos showing the completed fix or
                            resolution
                          </p>
                        </div>
                      </div>
